defmodule RumbleWeb.AuthTest do
  use RumbleWeb.ConnCase
  alias RumbleWeb.Auth

  setup %{conn: conn} do
    conn =
      conn
      |> bypass_through(RumbleWeb.Router, :browser)
      |> get("/")
  ...
  test "authenticate_user continues when the current_user exists", %{conn: conn} do
    conn =
      conn
      |> assign(:current_user, %Rumble.Accounts.User{})
  ...
  test "login puts the user in the session", %{conn: conn} do
    login_conn =
      conn
      |> Auth.login(%Rumble.Accounts.User{id: 123})
      |> send_resp(:ok, "")
